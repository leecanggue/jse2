...
- 이런경우 Parent의 메소드는 무시됨
- 같은 메소드가 Parent 와 Child에게 모두 선언 되었으나
- 내용은 다름
- 클래스의 다형성을 구현할 수 있음
- Parent 와 Child 간에 메소드의 원형(메소드명, 파라미터 갯수, 파라미터 데이터이 같아야함) 
- Parent 가 메소드를 상속해주나 Child는 자신이 구현한 메소드를 우선 사용.
- Parent 가 메소드 기능을 유지하면서 상황에 따라 Child의 변형된
...
public class OverridingDemo {
	/*
	Parent 와 show() 와 Child 와
	show() 원형이 같으므로 오버라이딩 이다.
	*/
